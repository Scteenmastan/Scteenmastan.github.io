...
function showCorrectById(id) {
    const stringList = ['Awesome', 'Excellent', 'Amazing', 'Good'];
    var text = stringList[Math.floor(Math.random() * stringList.length)];
    document.querySelector("#question" + id + " > div.collapse.answer > span > span.body").innerHTML = "<span class='magic-text'>" + text + "!</span>";
    document.querySelector("#question" + id + " > div.collapse.answer > span > span.body").style.display = 'block';
}

function showWrongById(id) {
    var text = '<span class="type-animation animating">Oops......Something&nbspwent&nbspwrong. The&nbspcorrect&nbspanswer&nbspis:&nbsp&nbsp' + getAnswerById(id) + '.</span>';
    document.querySelector("#question" + id + " > div.collapse.answer > span > span.body").innerHTML = text;
    document.querySelector("#question" + id + " > div.collapse.answer > span > span.body").style.display = 'block';
}
...
for (let i = 1; i <= 757; i++) {
    document.querySelector("#question" + i + " > div.collapse.answer > span").click();
}
